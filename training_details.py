...
# create="CREATE TABLE training_details(resid int(5),program char(120),Location char(30),Organization char(30),start_year char(10),end_year char(10),desc char(150));"
#
                ...
                if estart:
                    if eend:
                        if len(edesc)!=0:
                            if len(words) <= 45:
                                insert = (
                                    "INSERT INTO training_details(resid,program,Location,Organization,start_year,end_year,desc) values(?,?,?,?,?,?,?)")

                                cursor.execute(insert, [resid, eprogram, eloc, eorg, estart, eend, edesc])
                                sqlcon.commit()
                                messagebox.showinfo("Success!", "Your training details has been saved successfully :)")
                                clearr()
                            else:
                                messagebox.showinfo("Note!", "You cannot write more than 45 words")
                        else:
                            messagebox.showerror("Error!", "Please enter your training description")
                    else:
                        messagebox.showerror("Error!", "Please enter your ending month")
                else:
                    messagebox.showerror("Error!", "Please enter your start month")
            else:
                messagebox.showerror("Error!", "Please enter your training Organization")
        else:
            messagebox.showerror("Error!", "Please enter your training Loation")
    else:
        messagebox.showerror("Error!", "Please enter your training program")
    window.destroy()
...
background_img = PhotoImage(file = f"images/trbackground.png")
